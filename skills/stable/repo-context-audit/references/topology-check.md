# Topology check — Gate 2 of route-promote.md
...
```bash
<this skill's base directory>/scripts/derive-topology-vocab.sh
```

The script holds the derivation, the filter, and the two limits of that filter —
short nicknames are missed, and the system wordlist may be absent. Read its header
before trusting its output; it is the only argument for why an unfiltered list is
worse than no list at all. Re-run it when the fleet changes. Nothing consumes the
file but the check, and the check re-reads it on every run.
