...
|---|---|---|---|
| my cart is empty after I refresh the page and I never removed anything | [CART_EMPTIES_ON_REFRESH](../../../docs/traps/CART_EMPTIES_ON_REFRESH.md) | checkout | 2020-01-01 |
...
|---|---|---|
| [Currency codes](../../../docs/reference/currency-codes.md) | the six codes the payment provider accepts, and the two it silently drops | 2020-01-01 |
...
- [One currency per order](../../../docs/adr/0001-one-currency-per-order.md)
...
2. **Nothing cheaper catches it.** A doc is the *last* resort, because it only
   ...
   |---|---|---|
   | make it a **type error** | the mistake is expressible in the type system | <the type-check command, or "none"> |
   | make it a **test** | the mistake is an assertable behaviour | <the test command, or "there is no test runner"> |
   | **comment at the site** | there is exactly one line where someone could get it wrong | <is this the norm here? cite a good example> |
...
- **Regenerate the index**: `<the repo's index generator>`. The block above is
